# Text agent HTTP contract

For voice-mode evals, see the [voice agent contract](./voice-agent-contract.md). Overview of both modes: [agent contracts](./agent-contract.md).
...
The platform stores the full URL on the `Agent` record (e.g. `https://your-host/agent/respond`). The path segment `/agent/respond` is the recommended convention used by the reference [mock text agent](../examples/mock-text-agent/).
...
- **Reference implementation:** run the mock text agent (`examples/mock-text-agent/`) and open its `/docs` — FastAPI generates OpenAPI from the same request/response shapes.
...
| Path | Purpose |
|------|---------|
| [examples/mock-text-agent/](../examples/mock-text-agent/) | Tiny FastAPI text agent for local testing and onboarding. |
