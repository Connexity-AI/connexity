...
async def create_run(
    ...
    agent = crud.get_agent(
        session=session, agent_id=run_in.agent_id, company_id=company_id
    )
    if not agent:
        raise HTTPException(status_code=404, detail="Agent not found")
    eval_config = crud.get_eval_config(
        session=session, eval_config_id=run_in.eval_config_id, company_id=company_id
    )
    if not eval_config:
...
async def compare_runs_endpoint(
    ...
    baseline = crud.get_run(
        session=session, run_id=baseline_run_id, company_id=company_id
    )
    if not baseline:
        raise HTTPException(status_code=404, detail="Baseline run not found")
    candidate = crud.get_run(
        session=session, run_id=candidate_run_id, company_id=company_id
    )
    if not candidate:
...
async def compare_suggestions_endpoint(
    ...
    """
    baseline = crud.get_run(
        session=session, run_id=body.baseline_run_id, company_id=company_id
    )
    if not baseline:
        raise HTTPException(status_code=404, detail="Baseline run not found")
    candidate = crud.get_run(
        session=session, run_id=body.candidate_run_id, company_id=company_id
    )
    if not candidate:
        raise HTTPException(status_code=404, detail="Candidate run not found")
...
def get_run(session: SessionDep, company_id: CurrentCompany, run_id: uuid.UUID) -> Run:
    run = crud.get_run(session=session, run_id=run_id, company_id=company_id)
    if not run:
...
def delete_run(
    session: SessionDep, company_id: CurrentCompany, run_id: uuid.UUID
) -> Message:
    run = crud.get_run(session=session, run_id=run_id, company_id=company_id)
    if not run:
        raise HTTPException(status_code=404, detail="Run not found")
