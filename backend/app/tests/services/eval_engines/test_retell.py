...
def test_validate_config_raises_when_no_integration() -> None:
    ...
    with pytest.raises(ValueError, match="Retell environment"):
...
async def test_test_connection_when_integration_missing() -> None:
    ...
    assert "Retell environment" in result.message
...
async def test_run_test_case_creates_call_polls_and_judges() -> None:
    ...
    # Stub Environment + Integration lookups.
    fake_env = MagicMock()
    fake_env.integration_id = uuid.uuid4()
    fake_env.platform_agent_id = "retell_agent_123"
    session.exec.return_value.first.return_value = fake_env
