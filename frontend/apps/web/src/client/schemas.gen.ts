...
export const AgentCreateSchema = {
    properties: {
        ...
        platform: {
            ...
            description: 'Voice/agent platform this agent targets. Drives which evaluation engines are available. Null for legacy rows; use \'webhook\' for custom HTTP agents.'
        },
        prompt_type: {
        ...
        editor_guidelines: {
            ...
            description: 'Custom prompting guidelines for the prompt editor agent (None = use built-in default)'
        }
    },
    type: 'object',
    required: [
...
export const AggregateMetricsSchema = {
    properties: {
        ...
        weighted_metrics_score_pct: {
            ...
            description: 'Run-level metrics score (0-100): mean of per-test-case weighted overall_score across results that have a verdict. None when no result produced a verdict.'
        },
        ...
        metrics_passed: {
            ...
            description: 'True when weighted_metrics_score_pct >= metrics_pass_threshold. None when threshold or score is unavailable.'
        },
        cases_pass_rate_pct: {
            anyOf: [
                {
            ...
            description: 'Cases dimension score (0-100): pass_rate * 100. Errored test cases count as not-passed in the denominator.'
        },
        ...
        cases_passed: {
            ...
            title: 'Cases Passed',
            description: 'True when cases_pass_rate_pct >= cases_pass_threshold. None when threshold or rate is unavailable.'
        }
    },
    type: 'object',
    required: [
...
export const EnvironmentCreateSchema = {
    properties: {
        ...
        eval_gate_eval_config_id: {
            ...
            description: 'Optional: gate deploys on a passing run of this eval config for the requested agent version.'
        }
    },
    type: 'object',
    required: [
...
export const EvalConfigMemberEntrySchema = {
    ...
    description: 'API payload for adding or replacing config members with per-test-case repetition counts.'
} as const;
...
export const ExpectedToolCallSchema = {
    properties: {
        ...
        expected_params: {
            ...
            description: 'Used for mock routing (required argument keys must be present in the live call; values ignored) and generator alignment; null matches any invocation. Use {{paramName}} tokens for simulator- or agent-varying values.'
        },
        mock_response: {
            anyOf: [
                {
            ...
            title: 'Mock Response',
            description: 'Canned JSON object returned as the tool result in mock mode. Use one expected_tool_calls row per mocked invocation when the same tool is called multiple times.'
        }
    },
    type: 'object',
    required: [
...
export const MetricScoreSchema = {
    properties: {
        ...
        label: {
            type: 'string',
            title: 'Label',
            description: 'Scored: critical_fail, fail, poor, acceptable, good, excellent. Binary: pass or fail'
        },
        weight: {
            type: 'number',
            title: 'Weight',
...
export const PromptEditorChatMessageCreateSchema = {
    properties: {
        ...
        provider: {
            ...
            title: 'Provider',
            description: 'Optional LLM provider for this turn (e.g. openai, anthropic). Merged with model per LiteLLM rules; omit if model is a full routing id (contains /).'
        },
        model: {
            anyOf: [
                {
            ...
            title: 'Model',
            description: 'Optional LLM model for this turn (bare id or full vendor/model routing id). When omitted, server defaults apply.'
        },
...
export const PromptEditorSessionBasePromptUpdateSchema = {
    properties: {
        base_prompt: {
            type: 'string',
            title: 'Base Prompt',
            description: 'New diff baseline for this session (e.g. after saving the agent draft). Typically matches the current draft system_prompt.'
        }
    },
    type: 'object',
    required: [
...
export const RunConfig_InputSchema = {
    properties: {
        ...
        metrics_pass_threshold: {
            ...
            minimum: 0,
            title: 'Metrics Pass Threshold',
            description: 'Run-level threshold (%) for the weighted-average metric score across all test case executions. The run\'s metrics dimension passes when the average score is at or above this threshold.',
            default: 80
        },
        cases_pass_threshold: {
            type: 'number',
            ...
            minimum: 0,
            title: 'Cases Pass Threshold',
            description: 'Run-level threshold (%) for the fraction of test cases that pass. A test case passes when all of its expected_outcomes pass (or, for legacy test cases without expected_outcomes, when the judge verdict passes).',
            default: 100
        },
        judge: {
            anyOf: [
        ...
        user_simulator: {
            ...
            description: 'User simulator: LLM vs scripted replay, model/provider overrides, temperature. Omitted fields use app LLM defaults.'
        },
        agent_simulator: {
            anyOf: [
                {
            ...
            description: 'Agent simulator LLM overrides. Applies when the selected text runtime uses AgentSimulator (Connexity).'
        },
        mode: {
...
export const RunCreateSchema = {
    properties: {
        ...
        agent_version: {
            ...
            title: 'Agent Version',
            description: 'Target agent version to evaluate. If omitted, defaults to the agent\'s current version. If provided, the run is snapshotted from that AgentVersion row (system_prompt, tools, model, etc.).'
        },
        agent_version_id: {
            anyOf: [
                {
...
export const TestCaseCreateSchema = {
    properties: {
        ...
        persona_context: {
            ...
            description: 'Free-form persona description for the LLM simulator (type, description, behavioral instructions in one text block)'
        },
        first_turn: {
        ...
        first_message: {
            ...
            title: 'First Message',
            description: 'Opening message for whoever speaks first. When first_turn=user this is the user\'s opener; when first_turn=agent this is the agent\'s greeting.'
        },
        user_context: {
            anyOf: [
                {
        ...
        expected_outcomes: {
            ...
            description: 'List of true-statement assertions the judge evaluates (e.g. "Agent MUST confirm the appointment date")'
        },
        expected_tool_calls: {
            anyOf: [
                {
...
export const TestCaseImportItemSchema = {
    ...
    description: 'Test case payload for import — optional id enables round-trip and conflict detection.'
} as const;
